# React Counter App With Firebase Authentication
...
![Counter App](/src//assets/images/count.png "Counter")

This project is a solution to task given by [AltSchool Africa](https://www.altschoolafrica.com/). It is a simple counter app with a set count of zero ("0") that can take in a number from the input field, it can also increment, decrement and reset the count. It has a navigation bar with an active state as seen below that can be used to navigate to the home page, the 404 page (as instructed), and the Error Boundary page (as instructed).
...
This project has four major features:
...
This feature is created using firebase auth functionalities and useContext; this feature is used to authenticate users through email and password or through google. When the user signs up, an athentication mail is sent to the user's mailbox.
...
This feature allows the user to either sign up with an email and password or with their google account, and also allows the user to log out of the app.

![Counter App](/src/assets/images/counter%20signIn.png "sign up page")

After the user has signed up, the user can then login with the same email and password or with their google account

![Counter App](/src/assets/images/counter%20logIn.png "login page")

if a user is authenticated and logged in, the user can access the counter app, else the user will be redirected to the login page. If the user logs in with a google account, it displays the user's name and photo; but if the user logs in with an email and a password, it displays only the user's email address.
